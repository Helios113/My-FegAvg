...
data1 = pd.read_csv(os.path.join(main_path, cmp1, tar), header=None)
n = 1
# with open(os.path.join(main_path, cmp1, info), 'r') as f:
#     lines = f.read().splitlines()
#     last_line = lines[-1]
#     n = float(last_line.split(":")[-1].replace(" ", ""))

data2 = pd.read_csv(os.path.join(main_path, cmp2, tar), header=None)

plotSize = (6 * 1.618, 6)
fig, ax = plt.subplots(figsize=plotSize)
x_mean = data1.iloc[:, 3:-1:2].values
x_std = data1.iloc[:, 4::2].values
...
x_mean = data2.iloc[:, 3:-1:2].values
x_std = data2.iloc[:, 4::2].values
...
for i, v in enumerate(maxs1):
    max_ind = data1[data1[i*2+3]==v].index
...
for i, v in enumerate(maxs2):
    max_ind = data2[data2[i*2+3]==v].index
    sigs2.append(data2[i*2+4][max_ind].min())
